...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(num_channels)):
            ...
            self.level_index[i] = len(blocks) - 1
        ...
        self.level_index[-1] = len(blocks) - 1

        self.blocks = nn.ModuleList(blocks)

    def forward(self, x: torch.Tensor, levels=None) -> torch.Tensor:
        levels_output = []
        for idx, block in enumerate(self.blocks):
            if self.training:
                x = auto_grad_checkpoint(block, x)
                if levels and idx in [self.level_index[p] for p in levels]:
                    levels_output.append(x)
            else:
                x = block(x)
                if levels and idx in [self.level_index[p] for p in levels]:
                    levels_output.append(x)
        if levels:
            return x, levels_output
        return x
